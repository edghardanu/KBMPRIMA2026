    ...
    useEffect(() => {
        ...
        const fetchRecap = async () => {
            ...
            const { data: absensiData } = await supabase
                .from('absensi')
                .select('murid_id, status')
                .eq('jenjang_id', selectedJenjang);

            const recapMap: Record<string, AbsensiRecap> = {};
            muridData.forEach((m: any) => {
                recapMap[m.id] = { murid_id: m.id, nama: m.nama, hadir: 0, tidak_hadir: 0, izin: 0, sakit: 0, total: 0 };
            });

            absensiData?.forEach((a: any) => {
                if (recapMap[a.murid_id]) {
                    recapMap[a.murid_id][a.status as keyof Pick<AbsensiRecap, 'hadir' | 'tidak_hadir' | 'izin' | 'sakit'>]++;
                    recapMap[a.murid_id].total++;
                }
            });
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-stone-900">Rekap Absensi</h2>
                    <p className="text-stone-700 text-sm">Lihat rekapan absensi murid per jenjang</p>
                </div>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-stone-300">
                                    ...
                                    <th className="text-center px-6 py-4 text-xs font-medium text-stone-700 uppercase">Hadir</th>
                                    <th className="text-center px-6 py-4 text-xs font-medium text-stone-700 uppercase">Tidak Hadir</th>
                                    <th className="text-center px-6 py-4 text-xs font-medium text-stone-700 uppercase">Izin</th>
                                    <th className="text-center px-6 py-4 text-xs font-medium text-stone-700 uppercase">Sakit</th>
                                    <th className="text-center px-6 py-4 text-xs font-medium text-stone-700 uppercase">Total</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-stone-300">
                                {recap.map((r, i) => (
                                    <tr key={r.murid_id} className="hover:bg-[#f8f9f5]">
                                        <td className="px-6 py-4 text-sm text-stone-700">{i + 1}</td>
                                        <td className="px-6 py-4 text-sm text-stone-900 font-medium">{r.nama}</td>
                                        <td className="px-6 py-4 text-sm text-emerald-600 text-center">{r.hadir}</td>
                                        <td className="px-6 py-4 text-sm text-red-400 text-center">{r.tidak_hadir}</td>
                                        <td className="px-6 py-4 text-sm text-amber-400 text-center">{r.izin}</td>
                                        <td className="px-6 py-4 text-sm text-emerald-600 text-center">{r.sakit}</td>
                                        <td className="px-6 py-4 text-sm text-stone-900 text-center font-semibold">{r.total}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    </div>
                    {recap.length === 0 && <div className="text-center py-12 text-stone-700">Belum ada data absensi untuk jenjang ini</div>}
                </div>
            )}
        </div>
    );
